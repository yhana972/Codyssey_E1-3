...
## 6.1 MAC 점수가 높으면 왜 더 비슷하다고 보는가?

필터는 찾고 싶은 모양을 숫자로 표현한 기준입니다. 입력 패턴이 필터와 같은 위치에서 비슷한 값을 많이 가지고 있으면, 같은 위치끼리 곱했을 때 더 큰 값들이 많이 더해집니다.

예를 들어 Cross 모양의 입력은 Cross 필터의 1 위치와 많이 겹치기 때문에 Cross MAC 점수가 커집니다. 반대로 X 모양의 입력은 X 필터의 1 위치와 많이 겹치기 때문에 X MAC 점수가 커집니다.

따라서 이 프로젝트에서는 두 필터의 MAC 점수를 비교해서 더 높은 점수를 받은 필터를 입력 패턴과 더 비슷한 모양으로 판단합니다.
...
## 9.1 epsilon 기준을 바꾸면 결과가 달라질 수 있는가?

epsilon은 동점으로 볼 범위를 정하는 정책값입니다. 따라서 이 값을 바꾸면 `UNDECIDED`가 되는 케이스가 달라질 수 있습니다.

예를 들어 점수 차이가 `1e-10`인 케이스가 있을 때, epsilon이 `1e-9`이면 동점으로 보아 `UNDECIDED`가 됩니다. 하지만 epsilon을 `1e-12`로 더 작게 잡으면 같은 케이스도 동점이 아니라 한쪽 필터의 승리로 판정될 수 있습니다.

즉 epsilon은 정답을 계산하는 공식 자체가 아니라, 부동소수점 오차와 거의 같은 점수를 어떻게 처리할지 정하는 비교 기준입니다. 이번 프로젝트에서는 작은 오차 때문에 억지로 Cross 또는 X를 선택하지 않도록 `1e-9`를 사용했습니다.
